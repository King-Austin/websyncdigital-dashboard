...
import { IcHome, IcGlobe, IcBar, IcFile, IcCard, IcBell } from '@/components/ui/Icons';
  ...
  return (
    <div style={{ display: 'flex', minHeight: '100vh', background: T.bg }}>
      ...
      <div style={{ marginLeft: 240, flex: 1, display: 'flex', flexDirection: 'column', minHeight: '100vh' }}>
        <PageHeader title={meta.title} sub={meta.sub} actions={
          <>
            <Link href="/client/notifications" style={{ position: 'relative', width: 36, height: 36, display: 'flex', alignItems: 'center', justifyContent: 'center', borderRadius: 8, background: T.elevated, border: `1px solid ${T.border}`, cursor: 'pointer', textDecoration: 'none' }}>
              <IcBell sz={16} col={T.textS} />
              <span style={{ position: 'absolute', top: 8, right: 8, width: 6, height: 6, borderRadius: '50%', background: T.accent }} />
            </Link>
            <Link href="/admin" style={{ padding: '6px 14px', background: T.elevated, border: `1px solid ${T.border}`, borderRadius: 8, color: T.textS, fontSize: 12, fontWeight: 600, cursor: 'pointer', fontFamily: 'Plus Jakarta Sans', textDecoration: 'none' }}>
              Admin View →
            </Link>
          </>
        } />
